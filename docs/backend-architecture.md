...
├── config/
│   └── env.js                              # ALL process.env reads (PORT, DB_*, JWT_*, GEODB_API_KEY, OPENTRIPMAP_API_KEY, AMADEUS_*)
├── database/
│   ├── db.js                               # mysql2 Pool
...
- `authenticate` middleware reads the `access_token` cookie (via `cookie-parser`), verifies the JWT, and sets `req.user = { id, email }`. Every non-public route uses it.
...
| 6xxxx | External providers | `CITY_PROVIDER_UNAVAILABLE` 60001·502, `ACTIVITY_PROVIDER_UNAVAILABLE` 60002·502, `PRICING_PROVIDER_UNAVAILABLE` 60003·502 — all of these should be caught internally by the `*.service.js` fallback logic and rarely actually reach the client |
...
All providers throw a named `ERRORS.*_PROVIDER_UNAVAILABLE` on failure; the owning `*.service.js` catches that and falls back rather than letting it reach the controller (guide §9).
...
3. **Auth** — signup/login/logout, JWT set as httpOnly cookie (no refresh token), `authenticate` middleware
...
9. Wire up frontend against all of the above
